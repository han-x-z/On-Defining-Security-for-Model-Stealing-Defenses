...
def main():
    ...
    with tqdm(total=len(test_loader.dataset)) as pbar:
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            #print(outputs)
            tensor_output = outputs[def_level].clone().detach()
            #print(tensor_output)
            probs_list.append(tensor_output) #训练数据集
    ...
    # 保存label_tensor为文件
    with open('logs/res/mnist/label_tensor.pickle', 'wb') as f:
        pickle.dump(label_list, f)
    # 保存probs_tensor为文件
    with open('logs/res/mnist/probs_tensor.pickle', 'wb') as f:
