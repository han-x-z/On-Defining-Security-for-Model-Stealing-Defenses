...
def get_labels(X_sub, blackbox, defense_level, params):
    ...
    print(X_sub_len)
    if params['defense'] == 'SG':
        for i in range(0, X_sub_len, label_batch):
            x_sub_batch = X_sub[i:i+label_batch]  # 获取批量样本
            score_batch = blackbox(to_var(torch.from_numpy(x_sub_batch)))[defense_level]
            score_batch = score_batch.data.cpu().numpy()
            scores.append(score_batch)
    elif params['defense'] == 'BCG':
        for i in range(X_sub_len):
            x_sub = X_sub[i:i+1]  # 逐个获取样本
...
def main():
    ...
    train_loader = DataLoader(trainset, batch_size=sub_init_size, shuffle=False, num_workers=params['num_workers'], drop_last=True)
    test_loader = DataLoader(testset, batch_size=sub_init_size, shuffle=False, num_workers=params['num_workers'], drop_last=True)
...
#    print(X_sub)
    if params['defense'] == 'SG':
        blackbox_dir1 = '{}/{}/'.format('./models/defender/ImageNette/', 'ND')   #TODO
        blackbox1 = Blackbox.from_modeldir(blackbox_dir1, 'ND', 0, device)
        y_sub, _ = get_labels(X_sub, blackbox1, 0, params)
#    print(y_sub)
    else :
    ...
    for aug_round in range(params['aug_rounds']):
        ...
        if aug_round < params['aug_rounds'] - 1:
            ...
            #if aug_round == 0:
                #np.save('./cifar/X_sub4500.npy', X_sub)
            #np.save('./cifar/X_sub_{}.npy'.format(aug_round), X_sub)
