...
from admis.utils.defense import selectiveMisinformation, noDefense, selfGenerator, bcGenerator
...
class Blackbox(object):
    def __init__(
        ...
        elif self.defense == "SG":
            self.defense_fn = selfGenerator(defense_levels)
        elif self.defense == "BCG":
            self.defense_fn = bcGenerator(defense_levels)
        else:
    ...
    def from_modeldir(
        ...
        if defense == "SM":
            ...
            model_def = model_def.to(device)
            print(
                "=> loaded checkpoint for CGAN model (used for SG)"
            )
        elif defense == "BCG":
            model_def_path = model_dir + "/model_poison.pt"
            if not rand_fhat:
            ...
            model_def = model_def.to(device)
            print(
                "=> loaded checkpoint for CGAN model (used for BCG)"
            )    
