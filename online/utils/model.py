...
def train_step(model, train_loader, criterion, optimizer, epoch, device, restored, task, log_interval=10,
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if restored:
            ...
            restorer.load_state_dict(torch.load(generator_path + '/Restorer_epoch_20.pth'))   #TODO
            #restorer.load_state_dict(torch.load(generator_path + '/Restorer.pth'))  # TODO
...
#            inputs, targets = sample[0].to(device), sample[1].to(device)  # or something similar
            inputs, targets = inputs.to(device), targets.to(device)
