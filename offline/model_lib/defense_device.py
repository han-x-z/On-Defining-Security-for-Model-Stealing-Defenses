...
from online.victim.bb_BCG_detached import bcGenerator_device
from online.victim.bb_SG_detached import SGenerator_device
    ...
    def __call__(self, x, defense=None):
        ...
        if defense is None:
            ...
            self.AM_bb = AM_device(self.model,
                                   delta_list=[0.3], # comma separated values specifying defense levels: delta(SM)
                                   num_classes=10)
            ...
            self.SG_bb = SGenerator_device(self.model, # comma separated values specifying defense levels: delta(SM)
                                   num_classes=43) #TODO
            y_mod = self.SG_bb(x, y)
            #print(y_mod)
            y_mod = list(y_mod.values())[0]
            #print(y_mod)
            return y_mod
            ...
            if not hasattr(self, 'EDM_bb'):
                ...
                self.dataset_tar = 'cifar10'
                # TODO: change dataset name
                self.EDM_bb = EDM_device(task=self.dataset_tar)
            ...
            if not hasattr(self, 'ReverseSigmoid_bb'):
                ...
                self.ReverseSigmoid_bb = ReverseSigmoid_device(self.model, beta=self.beta, gamma=self.gamma,
                                                               out_path='models/final_bb_dist/ImageNette-resnet34-reverse_sigmoid-beta0.8-gamma0.2-ImageNette-B50000-proxy_scratch-random'
            ...
            if not hasattr(self, 'MAD_bb'):
                ...
                self.MAD_bb = MAD_device(self.model, epsilon=epsilon, optim=optim, model_adv_proxy=None,
                                         ...
                                         out_path='models/final_bb_dist/ImageNette-resnet34-mad_l1-eps1.0-ImageNette-B50000-proxy_scratch-random',
