...
import numpy as np
import sys
sys.path.append("/workspace/D-DAE/offline")
...
def train(epoch_itrs, batch_size, nz, shadow, generator, restorer, device, optimizer, epoch, shadow_loader,
    ...
    pbar = tqdm(total=len(shadow_loader) * epoch_itrs * len(shadow))
    for i in range(epoch_itrs):
        for j in range(len(shadow)):
            for k, (data, target) in enumerate(shadow_loader):
                ...
                optimizer_R.step()
                pbar.update(1)
                pbar.set_postfix({'S_loss': loss_S.item()})
        if verbose:
...
def main():
    ...
    SHADOW_NUM = 1
    #TODO
    num_shadow = 1  # shadow model used to train GAN in one epoch
    SHADOW_PROP = 0.02
    ...
    for epoch in tqdm(range(1, args.epoch_itrs + 1), desc="Training Epochs"):
        # Train
        shadow = []
        ...
        train(N_EPOCH, 128, args.nz, shadow,
        ...
        #dist_defensed_l2 = test(args.nz, restorer, restorer,
                                #shadow, device, test_loader, epoch=epoch,
                                #defense=args.defense)
        ...
        torch.save(restorer.state_dict(), f'{save_path_G}/Restorer_epoch_{epoch}.pth')
